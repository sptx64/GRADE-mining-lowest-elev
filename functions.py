...
def to_surpac_str(df, z="min_topo") :
    ...
    for i in range(len(x)) :
        strxyz.append(f"{string_no}, {x[i]}, {y[i]}, {z[i]},")
...
def main_process(res, list_topo_up) :
    ...
    #read topos
    value=0
    for uploaded_file in list_topo_up:
        topo = clean_str(uploaded_file)
        f"... topo {value} cleaned", len(topo)
